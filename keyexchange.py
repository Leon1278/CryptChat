...
class DiffieHellman:
	...
	def gen_shared_key(self, other_contribution):
		...
		if self.check_other_public_key(other_contribution):
			...
			return hashlib.md5(str(self.shared_key).encode()).hexdigest()
		else:
			raise Exception("Bad public key from other party")

	def executeClient(self, socket):
		pubkey = self.gen_public_key()
		bytes = pubkey.to_bytes(350, 'big')
		other_bytes = socket.recv(1024)
		other_key = int.from_bytes(other_bytes, 'big')
		bool = self.check_other_public_key(other_key)
		if bool:
			print("Other key is checked")
		else:
			print("Invalid key!")
			sys.exit()
		socket.send(bytes)
		shared_key = self.gen_shared_key(other_key)
		key = shared_key.encode('utf-8')
		print(key)
		return key

	def executeServer(self, socket):
		pubkey = self.gen_public_key()
		bytes = pubkey.to_bytes(350, 'big')
		socket.send(bytes)
		other_bytes = socket.recv(1024)
